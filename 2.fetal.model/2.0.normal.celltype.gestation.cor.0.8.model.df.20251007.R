...
cor.gene <- unique(cor.matrix.sel$gene)
cor.cell.type <- unique(cor.matrix.sel$cell.type)#5 kinds of celltype
...
for (i in 1:length(cor.cell.type)){
  ...
  gene.sel <- cor.matrix.sel[cor.matrix.sel$cell.type == celltype.sel,]
...
save(df, file = "/share/home/yangjingyi/project/1.BD/up.load/2.model/2.0.output/2.0.amniocytes.with.celltype.cor.0.8.exp.0.1.df.20251007.Robj")
...
plot <- plot_grid(p[[1]],p[[2]],p[[3]],p[[4]],p[[5]],p[[6]],
                  p[[7]],p[[8]],p[[9]],p[[10]],ncol = 4)
ggsave(filename="2.1.celltype.cor.0.8.exp.mean.0.1.without.legend.10.10.20250711.pdf",plot,width = 16, height = 10)
...
save(df.test, file = "/share/home/yangjingyi/project/1.BD/up.load/2.model/2.0.output/2.0.amniocytes.test.with.celltype.cor.0.8.exp.0.1.df.20251007.Robj")
#############################################################amnio.FGR.cohort1###################
...
save(df.FGR.cohort1, file = "/share/home/yangjingyi/project/1.BD/up.load/2.model/2.0.output/2.0.amniocytes.FGR.cohort1.with.celltype.cor.0.8.exp.0.1.df.20251007.Robj")
#############################################################amnio.FGR.cohort1###################
load("/share/home/yangjingyi/project/1.BD/up.load/3.FGR/5.0.output/amniocytes.cohort2.FGR.Robj")
amniocytes.cell.type <- unique(amniocytes.FGR.cohort2@meta.data$singleR_labels)

matrix <- as.data.frame(as.matrix(amniocytes.FGR.cohort2[["RNA"]]@data))
matrix <- matrix[rownames(matrix) %in% cor.gene,]

metadata <- amniocytes.FGR.cohort2@meta.data
unique(metadata$orig.ident)
metadata <- metadata[,c("barcode", "orig.ident", "singleR_labels")]
colnames(metadata) <- c("barcode", "orig.ident", "cell.type")
metadata <- metadata[metadata$cell.type %in% cor.cell.type,]
...
df.FGR.cohort2$orig.ident <- unique(amniocytes.FGR.cohort2@meta.data$orig.ident)
...
for (i in 1:length(cor.cell.type)){
  ...
  for (m in 1:length(gene.sel)){
    ...
    df.FGR.cohort2 <- merge(df.FGR.cohort2,group_mean,by="orig.ident", all = T)
...
save(df.FGR.cohort2, file = "/share/home/yangjingyi/project/1.BD/up.load/2.model/2.0.output/2.0.amniocytes.FGR.cohort2.with.celltype.cor.0.8.exp.0.1.df.20251007.Robj")
