...
library(dplyr)
library(tidyr)
amnio.matrix <- read.csv("/share/home/yangjingyi/project/1.BD/up.load/5.doublecheck/3.4.output/3.4.amnio.matrix.2020.cell.epi.20251020.csv", row.names = 1)
fetal.matrix <- read.csv("/share/home/yangjingyi/project/1.BD/up.load/5.doublecheck/3.4.output/3.4.fetal.matrix.2020.cell.epi.20251020.csv", row.names = 1)

summ.matrix <-  data.frame(matrix(ncol = 4, nrow = 0)) 
colnames(summ.matrix) <- c("gestation.week", "num.fetal", "num.amnio", "num.overlap")

ges.week <- c("GW17", "GW20", "GW22", "GW24")
...
ggsave(filename="1.5.lung.gene.num.ges.20251026.pdf",p,width = 8, height = 8)
