...
######
amnio.matrix <-  data.frame(matrix(ncol = 1, nrow = 7)) 
colnames(amnio.matrix) <- c("gestation.week")
amnio.matrix$gestation.week <- c("GW17", "GW18", "GW19", "GW20", "GW21", "GW22", "GW24")

fetal.matrix <-  data.frame(matrix(ncol = 1, nrow = 7)) 
colnames(fetal.matrix) <- c("gestation.week")
fetal.matrix$gestation.week <- c("GW17", "GW18", "GW19", "GW20", "GW21", "GW22", "GW24")
...
for (m in 1:length(matrix.gene)){
  ...
  ##matrix.fetal
  matrix.fetal.tmp <- matrix.merge[matrix.merge$group == "Fetal.intestine",]
...
write.csv(x = fetal.matrix, row.names = TRUE, file = "1.5.fetal.matrix.2020.cell.epi.20251025.csv")
write.csv(x = amnio.matrix, row.names = TRUE, file = "1.5.amnio.matrix.2020.cell.epi.20251025.csv")
