rm(list = ls())
setwd("/share/home/yangjingyi/project/1.BD/up.load/3.FGR/2.0.output/")
...
metadata1$cell.line[which(metadata1$seurat_clusters %in% c(0,1,2,3,4,5,6,16,11,14,15))] <- "Epithelial.cells"
metadata1$cell.line[which(metadata1$seurat_clusters %in% c(7,8,9,10,12,13,17,18))] <- "Hematopoietic.cells"
...
save(amnio.hemato, file = "/share/home/yangjingyi/project/1.BD/up.load/3.FGR/2.0.output/2.0.cohort1.amnio.hemato.Robj")

amnio.epi <- subset(x = amniocytes.sum1, subset = cell.line == "Epithelial.cells")
table(amnio.epi@meta.data$cell.line)
save(amnio.epi, file = "/share/home/yangjingyi/project/1.BD/up.load/3.FGR/2.0.output/2.0.cohort1.amnio.epi.Robj")
