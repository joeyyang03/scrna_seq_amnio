...
#resolution 0.5#######################################################################################################
#######################################################################################################################
load("/share/home/yangjingyi/project/1.BD/up.load/3.FGR/1.0.output/1.0.amniocytes.integrated.harmony.cohort1.Robj")
metadata <- amniocytes.sum1@meta.data
metadata$barcode <- rownames(metadata)
amniocytes.sum1@meta.data <- metadata

amnio.epi.metadata <- read.csv("/share/home/yangjingyi/project/1.BD/up.load/3.FGR/4.0.output/4.0.cohort1.amnio.epi.metadata.res.0.5.20251007.csv",row.names = 1)
amnio.hemato.metadata <- read.csv("/share/home/yangjingyi/project/1.BD/up.load/3.FGR/4.0.output/4.0.cohort1.amnio.hemato.metadata.res.0.5.20251007.csv",row.names = 1)
...
gestation.day <- gestation.day[gestation.day$vali.group == "cohort1",]
...
amniocytes.FGR.cohort1 <- amniocytes.sum1
unique(amniocytes.FGR.cohort1@meta.data$gestation.day)
save(amniocytes.FGR.cohort1, file = "/share/home/yangjingyi/project/1.BD/up.load/3.FGR/5.0.output/amniocytes.cohort1.FGR.Robj")
...
for (i in 1:length(amniocytes.cell.type)){
  ...
  save(amniocytes.type, file = paste0("/share/home/yangjingyi/project/1.BD/up.load/3.FGR/5.0.output/5.0.cohort1.",a,".Robj"))
