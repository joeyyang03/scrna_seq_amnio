rm(list = ls())
setwd("/share/home/yangjingyi/project/1.BD/up.load/3.FGR/1.1.output/")
...
cohort2_dirs <- grep("^cohort2", dir, value = TRUE)

print(cohort2_dirs)

sample.list <- list()
for (i in 1:length(cohort2_dirs)){
  counts <- Read10X(paste0(folder_path,cohort2_dirs[[i]],"/"), unique.features = TRUE)
  sample.list[[i]] <- CreateSeuratObject(counts = counts, min.cells= 3, min.features = 200, project = cohort2_dirs[[i]])
  sample.list[[i]] <- RenameCells(sample.list[[i]], add.cell.id = cohort2_dirs[[i]])
  print(cohort2_dirs[[i]])
}
...
save.image(file = "/share/home/yangjingyi/project/1.BD/up.load/3.FGR/1.1.output/1.1.amniocytes.normal.list.cohort2.RData")
...
save(amniocytes.sum1, file = "/share/home/yangjingyi/project/1.BD/up.load/3.FGR/1.1.output/1.1.amniocytes.integrated.FindNeighbors.cohort2.Robj")

amniocytes.sum1 <- FindClusters(amniocytes.sum1, resolution = 0.4)


save.image(file = "/share/home/yangjingyi/project/1.BD/up.load/3.FGR/1.1.output/1.1.amniocytes.integrated.harmony.cohort2.RData")
save(amniocytes.sum1, file = "/share/home/yangjingyi/project/1.BD/up.load/3.FGR/1.1.output/1.1.amniocytes.integrated.harmony.cohort2.Robj")
