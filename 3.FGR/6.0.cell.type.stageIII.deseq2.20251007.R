...
amniocytes.normal.stageIII <- subset(amniocytes.type,subset = Trimester %in% c("Stage.III"))
amniocytes.normal.stageIII@meta.data$group <- "Normal"
###############################################################################################################################
####
...
amniocytes.FGR.stageIII <- subset(amniocytes.type, subset = Trimester %in% c("Stage.III"))
amniocytes.FGR.stageIII@meta.data$group <- "FGR"



################################################################################################################################
amniocytes.stageIII <- merge(amniocytes.normal.stageIII, amniocytes.FGR.stageIII)

metadata <- amniocytes.stageIII@meta.data
information <- metadata[,c("barcode", "group")]

merge <- amniocytes.stageIII@assays$RNA@counts
...
write.table(res, file = paste0("/share/home/yangjingyi/project/1.BD/up.load/3.FGR/6.0.output/6.0.deseq2.stageIII.",celltype,".20251007.txt"))


save(dds.1, file = paste0("/share/home/yangjingyi/project/1.BD/up.load/3.FGR/6.0.output/6.0.deseq2.stageIII.results.",celltype,".20251007.Robj"))
